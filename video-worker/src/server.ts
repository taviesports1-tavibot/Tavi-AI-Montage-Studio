...
export async function buildServer(dependencies: ServerDependencies = {}) {
  const app = Fastify({
    logger: false,
    bodyLimit:
      config.maxClipBytes * config.maxClips + config.maxMusicBytes,
    requestTimeout: 0,
  });
  const storage =
  ...
  app.get("/api/health", async () => {
    const ffmpeg = await runProcess(config.ffmpegPath, ["-version"], {
      timeoutMs: 10_000,
    });
    return {
      ok: true,
    ...
    try {
      for await (const part of request.parts()) {
        ...
        if (part.fieldname === "clips") {
          ...
          const extension = validateVideoUpload(
            part.filename,
            part.mimetype,
          );
        ...
        if (part.fieldname === "music") {
          ...
          const extension = validateMusicUpload(
            part.filename,
            part.mimetype,
          );
  ...
  app.get<{ Params: { jobId: string } }>(
    "/api/jobs/:jobId",
    async (request) => storage.readJob(request.params.jobId),
  );
  ...
  }>("/api/projects/:projectId/rerender", async (request, reply) => {
  ...
  app.get<{
    ...
    async (request, reply) => {
      ...
      const target = storage.getRenderPath(
        project.id,
